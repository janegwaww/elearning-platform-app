...
const Remin = () => {
  ...
  const update_data = (_type) => {
    setLoginstatus(true);
    get_data({
      extra_data: { max_size: 12, page: 1, type:_type|| tp },
      model_action: "hot_video",
      model_name: "video",
      model_type: "",
    })
      .then((res) => {
  ...
  return (
    <div>
      ...
      {showdata ? <Grid container spacing={2}>
          ...
          {showdata.length>0?(showdata.map((op)=><Grid item xs={6} key={op.video_id||op.file_id}>
            <Item info={op} />
          </Grid>)):<Nofile onEvent={update_data} />}
      
      </Grid> : <LoadData />}

      {showdata && showdata.length > 0 && (
        <div className="text-center" style={{ marginTop: 30 }}>
          <Button variant="contained"   className={classes.btn} color="primary" onClick={()=>{update_data(tp)}}>
            换一批
          </Button>
        </div>
      )}
